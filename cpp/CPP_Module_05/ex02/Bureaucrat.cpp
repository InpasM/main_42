...
/*   Updated: 2023/11/16 14:10:24 by msapin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::signForm(Form & formular) const {
	...
	{
		alreadySigned = true;
		std::cout << RED << this->_name << " couldn't Sign " << formular.getName()
	...
	if (formular.getIsSigned() && !alreadySigned)
		std::cout << this->_name << " signed " << formular.getName() << std::endl;
}

void	Bureaucrat::executeForm(Form & formular) const {
	...
	catch(Form::UnsignedException& e)
	{
		...
		<< formular.getName() << " because " << e.what() << RESET << std::endl;
	}
	catch(Form::GradeTooLowException& e)
	{
		executed = false;
	...
	catch(Form::FileAccessException& e)
	{
		executed = false;
		std::cout << RED << "ShrubberyCreationForm: " << e.what() << " "
		<< formular.getName() << " file!" << RESET << std::endl;
	}
	if (executed)
		std::cout << this->_name << " executed " << formular.getName() << std::endl;
}
